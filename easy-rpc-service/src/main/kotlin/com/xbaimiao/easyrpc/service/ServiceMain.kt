...
fun main() {
    ...
    val server = NettyRpcServer(host = config.host, port = config.port, nodeId = config.nodeId)

    BuiltinRpc.PING.listen(server) { text -> "pong:$text" }

    server.start()
    println("EasyRpc service started on ${config.host}:${config.port} (nodeId=${config.nodeId})")
